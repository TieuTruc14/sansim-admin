...
app.controller('sansimCtrl',['$scope','$http','LoadCustomerUsernameAuto'
    ,function ($scope,$http,LoadCustomerUsernameAuto) {
        ...
        $scope.search=function () {
            ...
            $scope.loadding=true;
            $http.get(preUrl+"/report/general-trade/search", {params: {type:$scope.type,username:$scope.username,from:$scope.from,to:$scope.to}})
        ...
        $scope.download=function () {
            window.open(preUrl+"/report/general-trade/download?type="+$scope.type+"&username="+$scope.username+"&from="+$scope.from+"&to="+$scope.to, '_blank');
        };
        ...
        $scope.loadPage=function (pageNumber) {
            if(pageNumber>=1){
                ...
                $http.get(preUrl+"/report/general-trade/search", {params: {p:pageNumber,type:$scope.type,username:$scope.username,from:$scope.from,to:$scope.to}})
                    .then(function (response) {
                        if(response!=null && response!='undefined' && response.status==200){
                            $scope.page=response.data;
                            $scope.page.pageCount=getPageCount($scope.page);
