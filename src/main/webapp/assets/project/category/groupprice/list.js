...
app.controller('sansimCtrl',['$scope','$http','$filter','$window','$timeout','$q'
    ,function ($scope,$http,$filter,$window,$timeout,$q) {
        ...
        $scope.page=page;
        $http.get(preUrl+"/category/group-price/list/search", {params: {name:$scope.name}})
        ...
        $scope.loadPage=function (pageNumber) {
            if(pageNumber>=1){
                $http.get(preUrl+"/category/group-price/list/search", {params: {p:pageNumber,name:$scope.name}})
                    .then(function (response) {
                        $scope.page=response.data;
                        $scope.page.pageList=getPageList($scope.page);
                        $scope.page.pageCount=getPageCount($scope.page);
        ...
        $scope.orderGroup=function () {
            ...
            $http.put(preUrl+"/category/group-price/order-list-all",listSapXep, {headers: {'Content-Type': 'application/json'} })
                .then(function (response) {
                        if(response!=null && response!='undefined' && response.status==200){
        ...
        $scope.reload=function () {
            $window.location.href=preUrl+"/category/group-price/list";
        };
